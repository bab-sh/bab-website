        ...
        <div class="border-border overflow-hidden rounded-xl border">
          ...
          >
            <div class="flex items-center gap-2">
              <SvgoBabLogo class="h-4 w-4" aria-label="bab logo" />
...
<script setup lang="ts">
  ...
  const lines: Line[] = [
    ...
    L.feature(
      [indent(6), punct('-'), indent(1), key('prompt'), punct(':'), indent(1), value('port')],
      'dev',
      'prompt',
    ),
    L.feature([indent(8), key('type'), punct(':'), indent(1), value('number')], 'dev', 'prompt'),
    L.feature(
      [indent(8), key('message'), punct(':'), indent(1), value('Enter dev server port')],
      'dev',
      'prompt',
    ),
    L.feature([indent(8), key('default'), punct(':'), indent(1), value('3000')], 'dev', 'prompt'),
    L.task(
      [
        indent(6),
        punct('-'),
